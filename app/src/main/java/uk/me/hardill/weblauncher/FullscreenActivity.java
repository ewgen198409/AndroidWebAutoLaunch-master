...
public class FullscreenActivity extends AppCompatActivity implements SwipeListerner{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("FullscreenActivity", "=== APP STARTED ===");

        setContentView(R.layout.activity_fullscreen);

        PreferenceManager.setDefaultValues(this, R.xml.preferences, false);
        sharedPref = PreferenceManager.getDefaultSharedPreferences(this);
        currentLanguage = sharedPref.getString("language", "en");
        currentTheme = sharedPref.getString("theme", "auto");
        applyTheme(currentTheme);

        // Auto-start DLNA renderer service if enabled
        boolean dlnaEnabled = sharedPref.getBoolean("enable_upnp_renderer", false);
        if (dlnaEnabled) {
            Log.i("FullscreenActivity", "Auto-starting DLNA renderer service");
            Intent serviceIntent = new Intent(this, UpnpAudioRendererService.class);
...
//        show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            ...
            boolean dlnaEnabled = sharedPref.getBoolean("enable_upnp_renderer", false);
            Log.i("Prefs", "DLNA renderer enabled: " + dlnaEnabled);

            // Check if language changed
            String newLanguage = sharedPref.getString("language", "en");
            if (!newLanguage.equals(this.currentLanguage)) {
                Log.i("Prefs", "Language changed to: " + newLanguage);
                // Restart entire app to apply new language
            ...
            String newTheme = sharedPref.getString("theme", "auto");
            if (!newTheme.equals(this.currentTheme)) {
                Log.i("Prefs", "Theme changed to: " + newTheme);
                applyTheme(newTheme);
                this.currentTheme = newTheme;
                // Restart entire app to apply new theme
                Intent intent = getBaseContext().getPackageManager().getLaunchIntentForPackage(getBaseContext().getPackageName());
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(intent);
